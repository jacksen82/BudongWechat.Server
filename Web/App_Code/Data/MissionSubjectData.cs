...
public class MissionSubjectData
{
    ...
    /// <param name="categoryName">string 题目分类名称</param>
    /// <param name="index">int 题目索引</param>
    /// <param name="mp3Url">string 音频文件路径</param>
    /// <returns>int 受影响的行数</returns>
    public static int Add(int clientId, int missionId, string title, string tip, int categoryId, string categoryName, int index, string mp3Url)
    {
        string sql = "INSERT INTO tm_mission_subject (clientId,missionId,title,tip,categoryId,categoryName,`index`,mp3Url) VALUES(@0,@1,@2,@3,@4,@5,@6,@7) ON DUPLICATE KEY UPDATE missionId=VALUES(missionId), title=VALUES(title)";
        using (MySqlADO ado = new MySqlADO())
        {
            return ado.NonQuery(sql, clientId, missionId, title, tip, categoryId, categoryName, index, mp3Url);
        }
    }
    /// <summary>
    /// 修改题目信息
    ...
    /// <param name="categoryName">string 题目分类名称</param>
    /// <returns>int 受影响的行数</returns>
    public static int Edit(int subjectId, string title, string tip, int categoryId, string categoryName)
    {
        string sql = "UPDATE tm_mission_subject SET title=@1,tip=@2,categoryId=@3,categoryName=@4 WHERE id=@0";
        using (MySqlADO ado = new MySqlADO())
        {
            return ado.NonQuery(sql, subjectId, title, tip, categoryId, categoryName);
        }
    }
    /// <summary>
    /// 删除题目
